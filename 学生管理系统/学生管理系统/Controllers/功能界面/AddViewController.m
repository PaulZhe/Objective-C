...
- (void)viewDidLoad {
    ...
    [_addButton setTitle:@"确定添加" forState:normal];
...
- (void)press{
    int flag = 1;
    StudentSystem *studentSystem = [StudentSystem sharedStudent];
    if([_ageTextField.text isEqualToString:@""] || [_nameTextField.text isEqualToString:@""] || [_stuIDTextField.text isEqualToString:@""] || [_classTextField.text isEqualToString:@""]|| [_scoreTextField.text isEqualToString:@""] || [_stuIDTextField.text isEqualToString:@"0"]){
        flag = 0;
    }
    ...
    if(flag == 1){
        ...
        student.stuID = [self.stuIDTextField.text intValue];
        student.name = self.nameTextField.text;
        student.age = [self.ageTextField.text intValue];
        student.classNum = [self.classTextField.text intValue];
        student.score = [self.scoreTextField.text intValue];
        [studentSystem->studentArray addObject:student];

        [self.tableView reloadData];
    } else{
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"添加失败" message:@"添加学号已存在或者有学生信息未录入" preferredStyle:UIAlertControllerStyleAlert];
